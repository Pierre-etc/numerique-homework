...
print(rectangle_size(4),rectangle_size(7))
```
...
def patchwork(colors, side=10, background=[169, 169, 169]):
    ...
    indices = np.arange(l*c)
    indices = indices.reshape(l,c)
    colors2 = np.concatenate((colors,np.array((l*c-n)*[background])))
    return colors2[indices]
```
...
def sepia(image):
    ...
    res = np.reshape(np.array([0.393*r+0.769*g+0.189*b,0.349*r+0.686*g+0.168*b,0.272*r+0.534*g+0.131*b]),(533,800,3))
    return res
```
...
```{code-cell} ipython3
# votre code
test = plt.imread("data/les-mines.jpg")
plt.imshow(sepia(test))
```
...
type(res1)
np.max(res1)
plt.imshow(res1, cmap = ("gist_gray"))
```
...
type(res2)
np.max(res2)
plt.imshow(res2, cmap = ("gist_gray"))
```
...
# la fonction np.sum adapte le dtype : si en additionnant on se retrouve avec un entier plus grand que la limite, le dtype est changé pour lui allouer plus de bits
# tandis que le + garde toujours le même dtype
```
...
res3[mask_sup] = 255
res3[mask_inf] = 0
plt.imshow(res3, cmap = "gist_gray")
```
...
```{code-cell} ipython3
# votre code
plt.imsave("data/image_sauvee_c.jpg",b)
a.save("data/image_sauvee_d.jpg",quality = 100)
```
...
```{code-cell} ipython3
# votre code
print(os.path.getsize("data/image_sauvee_c.jpg"))
print(os.path.getsize("data/image_sauvee_d.jpg"))

# la seconde pèse plus lourd en mémoire que la première
```
...
```{code-cell} ipython3
# votre code
c = plt.imread("data/image_sauvee_c.jpg")
d = img.open("data/image_sauvee_d.jpg")
...
```{code-cell} ipython3
# l'image d, celle enregistré avec PIL et dont on a choisi la qualité, est bien plus nette que l'image c
```
