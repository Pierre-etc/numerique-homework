...
```{code-cell} ipython3
# votre code
len(df['Pclass'].unique())*len(df['Survived'].unique())*len(df['Sex'].unique())
```
...
```{code-cell} ipython3
# votre code
partition = df.groupby(by = ['Pclass','Sex','Survived'])
partition.size()
```
...
```{code-cell} ipython3
# votre code
partition.get_group((1,'female',0))
```
...
res = {}
for (sex,classe),dtfr in by_sex_class:
    res[(sex,classe)] = dtfr['Survived'].mean()
res
```
...
```{code-cell} ipython3
# votre code
pd.Series(name = "taux de survie par genre dans chaque classe")
```
